...
void find_border(node** inner_border, int ninner,  node** outer_border, int* nborder, float* distances);
    ...
    *nborder=0;
    ...
    for(int i=0; i < ninner; i++){ //For inner border nodes
        ...
        for(int j=0; j<c_vtx->n_outgoing_edges; j++){
            ...
            if( e->travelled != TRUE && t_vtx->inside == FALSE){ //If we haven't travelled along this edge yet
                //Updated Distance of Target Node
                float dist=c_vtx->dist + e->length;
                if( dist < distances[t_vtx->index] || distances[t_vtx->index] == 0){
                    distances[t_vtx->index]=dist;
                    t_vtx->dist=dist;

                }
                    //Add target node to outer border
                    *nborder += 1;
                    outer_border[*nborder-1]=t_vtx;
            }
            e->travelled=TRUE;
        }
    }
    for(int i=0; i < *nborder; i++) outer_border[i]->inside=TRUE;

}

void wm_dist(data* img, node** mesh, int* img_vol,  node** gm_border, int n, int WM, int start, int step){
    ...
    int max =img->n3d;
    int wm_total=0;
    double min_step_size=img->step[0]+img->step[1]+img->step[3];
    ...
    for(int i=start; i < n; i += step){ //Iterate over nodes on WM-GM border
        if(i % 100 == 0) printf("%3.3f\r", (float) 100. * i/n);
        ...
        /**/
        while( nouter > 0){
            find_border(inner_border, ninner, outer_border, &nouter, distances);
            tmp=inner_border;
        ...
        for(int j=0; j < max; j++){ 
            if(mesh[j] != NULL) { 
                mesh[j]->dist=0; 
                mesh[j]->inside=FALSE;
                for(int k=0; k<mesh[j]->n_outgoing_edges; k++){
                    edge* e=mesh[j]->outgoing_edges[k];
                    e->travelled=FALSE;
                }
            }
        }
...
void* wm_dist_threaded(void* args){
    ...
    wm_dist(img,  mesh, img_vol,  gm_border, n,  WM, thread, nthreads);
}

int wm_dist_multithreaded(data* img, node** mesh, int* img_vol, node** gm_border, int WM, float* mat, int n, int nthreads){
    int rc;
...
int main(int argc, char** argv){
    ...
    for(int j=0; j < nvertices; j++) if(mesh[j] != NULL) if(mesh[j]->border == TRUE) gm_border[k++]=mesh[j];
    int wm_total=0;
    for(int j=0; j < nvertices; j++) if(mesh[j] != NULL)if(mesh[j]->val == WM) wm_total++;
    /*Find voxel to voxel distances through the WM*/
    wm_dist(&anatomy, mesh, img_vol, gm_border, n, WM, 0, 1);
    //wm_dist_multithreaded(&anatomy, mesh, img_vol, gm_border, WM, mat, n, nthreads);
