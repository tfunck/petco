...
int idSURF_multithread(data* image,double* test,double* residuals,double* noise,double* observed,double* blurred,mihandle_t* outputimage,misize_t* sizes,misize_t* starts, float* temp, int*** regions, int *n_avg_region, double* region_t_weights, int* mask_label,  int max_iterations, double fwhm, int p, double tolerance, data* masks, int nmasks, int max_avg, int smooth_only, data* first_guess, int nthreads){
    int rc;
    ...
    for(int t=0; t< nthreads; t++){
        ...
        thread_args[t].region_t_weights=region_t_weights;
        ...
        thread_args[t].test=test;
        thread_args[t].residuals=residuals;
        thread_args[t].noise=noise;
        thread_args[t].observed=observed;
        thread_args[t].blurred=blurred;
        thread_args[t].outputimage=outputimage;
        thread_args[t].sizes=sizes;
        thread_args[t].starts=starts;
        thread_args[t].temp=temp;


        thread_args[t].t=t;
        rc= pthread_create(&threads[t], NULL, idSURF, (void *) &thread_args[t] ); //gradient(image, img_dx, img_dy, img_dz);
    }
    for(int t=0; t<nthreads;t++){
...
int idSURF(void* args){ 
    ...
    double* region_t_weights=((struct idargs*) args)->region_t_weights; 
    ...
    int nmasks=((struct idargs*) args)->nmasks; 
    ...
    double* test=((struct idargs*) args)->test;
    double* residuals=((struct idargs*) args)->residuals;
    double* noise=((struct idargs*) args)->noise;
    double* blurred=((struct idargs*) args)->blurred;
    misize_t *sizes=((struct idargs*) args)->sizes;
    misize_t *starts=((struct idargs*) args)->starts;
...
int main(int argc, char** argv){      
    ...
    //Allocate memory
    test=malloc(nvox3d*sizeof(*test));
    residuals=calloc(nvox3d,sizeof(*residuals));
    noise=calloc(nvox3d,sizeof(*noise));
    blurred=malloc(nvox3d*sizeof(*blurred));
    sizes=calloc(image->ndim, sizeof(*sizes));
    starts=calloc(image->ndim , sizeof(*starts));
    temp=malloc(nvox3d*sizeof(*temp));
    ...
    idSURF_multithread(image, test, residuals, noise, observed, blurred, &outputimage, sizes, starts, temp_array,  avg_region, n_avg_region, avg_region_weights, mask_label, max_iterations, fwhm,  p, tolerance, masks, nmasks, max_avg, smooth_only, first_guess, nthreads);
