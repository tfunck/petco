...
int find(int x, int* list, int n){
    for(int i=0; i< n; i++) if( x == list[i] )  return(i); //find first element in list that equals x
    return(-1);//and return the index number of that element
}
...
*Output:    
*********************************************************************************************/
int find_cubic_averaging_region(data* img, int*** region, int* n_region, int nlabels, int* labels,  int* mask, int max_ngh){
    int nvoxels=img->n;
    int nngh;
...
*********************************************************************************************/
int find_averaging_region(int*** region, int* n_region, int* mask_label, int **ngh, int* nngh,int nvoxels,  int nlabels, int* labels, int* mask, int max_ngh){
...
*Output:    
*********************************************************************************************/
int find_neighbours(int max_ngh, int** ngh, int* nngh, int* mask_label, int* mask_data, int nlabels, int* labels, data* mask){
    //variables
...
int idSURF(data* image, char* outputfilename, int*** regions, int *n_avg_region, double* region_t_weights, int* mask_label,  int max_iterations, double fwhm, int p, double tolerance, data* masks, int nmasks, int max_avg, int nlabels, int* labels, int smooth_only, data* first_guess){
...
int main(int argc, char** argv){      
    ...
    //read in mask volume 
    int* mask_data=(int*) readVolume(masks, 1, MI_TYPE_INT );
    if( mask_data == NULL  ) pexit("Could not read data from" , masks->filename,  1);
...
void useage(){
        printf("\nName:\n\tidSURF-4.1.2 -- partial-volume correct PET image using masks of anatomical regions\n");
        printf("Description:\n");
