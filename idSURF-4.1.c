...
int read_frame(data* image, int t);
void useage();
...
int idSURF(data* image, char* outputfilename, int*** regions, int *n_avg_region, double* region_t_weights, int* mask_label,  int max_iterations, double fwhm, int p, double tolerance, data* masks, int nmasks, int max_avg, int smooth_only, data* first_guess){
    ...
    miopen_volume("observed.mnc", MI2_OPEN_RDWR, &output_observed_image);
    float sum=0;
    int nlabels=0;
    ...
    for(int t=0; t < max_frames; t++){
        ...
        //writeVolume("observed.mnc", observed, start_3d, step_3d,  wcount_3d, MI_TYPE_DOUBLE  );
        double initial_energy=0, old_max=0;
        if(first_guess->filename != NULL) read_frame(first_guess, t); 
        ...
        **********************/
        ...
        miset_real_value_hyperslab(output_observed_image, MI_TYPE_DOUBLE, starts, sizes, observed);

    }
    miclose_volume(outputimage);
    miclose_volume(output_observed_image);
    return(0);
}
...
int main(int argc, char** argv){      
    ...
    createVolume(outputfilename, image->ndim, image->count, image->step, image->start);
    createVolume("observed.mnc", image->ndim, image->count, image->step, image->start);
